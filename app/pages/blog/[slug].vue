<template>
  <div class="claude-container py-12">
    <button
      @click="router.back()"
      class="flex items-center text-primary hover:underline mb-6"
    >
      <svg
...
const fetchBlogPost = async () => {
  try {
    ...
    const response = await fetch(`/api/blog-post?id=${postId}`);
    const data = await response.json();

    if (data.success) {
      post.value = data.post;
    } else {
      error.value = data.error || "Failed to fetch blog post";
    }
  } catch (err) {
  ...
  meta: [
    {
      name: "description",
      content: computed(
        () => post.value?.content?.substring(0, 160) + "..." || "Blog post"
      ),
    },
  ],
});
